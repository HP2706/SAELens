...
from parsing import decompose_attr, get_nested_attr
...
args = [(model, input, a, b) for (a, b) in test_cases]


@pytest.mark.parametrize("model, input, attr_str,fn", args)
def test(model: NNsight, input: Any, attr_str: str, fn: Callable[[NNsight], Any]):
    ...
    assert (
        saved_value1.value.shape == saved_value2.value.shape
    ), f"shape are not the same, got: saved_value1.shape={saved_value1.value.shape} and saved_value2.shape={saved_value2.value.shape}"
